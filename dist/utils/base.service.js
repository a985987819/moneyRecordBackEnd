 ...
 * 从数据库行安全地提取浮点数
 */
export function safeRowFloat(row, key, defaultValue = 0) {
 ...
 * 从数据库行安全地提取整数
 */
export function safeRowInt(row, key, defaultValue = 0) {
...
export class BaseService {
     ...
     * @returns 映射后的对象
     */
    mapRowToResponse(row, mapping) {
        return mapRow(row, mapping);
    }
    /**
     * 安全获取浮点数值
     */
    getFloat(row, key, defaultValue = 0) {
        return safeRowFloat(row, key, defaultValue);
    }
    /**
     * 安全获取整数值
     */
    getInt(row, key, defaultValue = 0) {
