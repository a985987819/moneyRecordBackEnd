...
export class DebtService extends BaseService {
    ...
    async getAllDebts(userId) {
        const result = await db.query(`SELECT id::text, type, person_name, amount, repaid_amount, remaining_amount,
    ...
    async createDebt(userId, data) {
        const result = await db.query(`INSERT INTO debts
    ...
    async updateDebt(userId, id, data) {
        const result = await db.query(`UPDATE debts
    ...
    async deleteDebt(userId, id) {
        const result = await db.query(`DELETE FROM debts WHERE id = $1 AND user_id = $2 RETURNING id`, [id, userId]);
        return result.rowCount ? result.rowCount > 0 : false;
    }
    // 记录还款
    async repay(userId, id, data) {
        const client = await db.getClient();
        try {
            await client.query('BEGIN');
            // 获取当前借贷信息
            const currentResult = await client.query(`SELECT amount, repaid_amount FROM debts WHERE id = $1 AND user_id = $2`, [id, userId]);
            ...
            const result = await client.query(`UPDATE debts
         SET repaid_amount = $1,
             remaining_amount = $2,
             status = $3,
             updated_at = CURRENT_TIMESTAMP
         ...
         RETURNING id::text, type, person_name, amount, repaid_amount, remaining_amount,
                   date, expected_repay_date, remark, status,
                   TO_CHAR(created_at, 'YYYY-MM-DD HH24:MI:SS') as created_at,
                   TO_CHAR(updated_at, 'YYYY-MM-DD HH24:MI:SS') as updated_at`, [newRepaidAmount, remainingAmount, status, id, userId]);
            // 记录还款历史
            await client.query(`INSERT INTO debt_repayments (debt_id, amount, remark, user_id)
         VALUES ($1, $2, $3, $4)`, [id, data.amount, data.remark || null, userId]);
