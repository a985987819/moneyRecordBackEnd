...
export class RecordService extends BaseService {
    async getMonthlyStats(userId, month) {
       ...
       FROM records 
       WHERE user_id = $1 AND TO_CHAR(date, 'YYYY-MM') = $2`, [userId, targetMonth]);
    ...
    async getRecentRecords(userId) {
       ...
       LIMIT 50`, [userId, threeDaysAgo.toISOString().split('T')[0], today.toISOString().split('T')[0]]);
        return result.rows;
    }
    async getRecords(userId, params) {
        let query = `
      SELECT
        id::text, type, category, sub_category as "subCategory", category_icon as "categoryIcon",
        amount, remark, TO_CHAR(date, 'YYYY-MM-DD HH24:MI:SS') as date, account, is_import as "isImport"
    ...
    async getRecordsByDatePaginated(userId, cursor, limit = 10) {
        // 如果没有 cursor，使用明天作为边界，确保能获取到今天的记录
        const tomorrow = new Date();
        tomorrow.setDate(tomorrow.getDate() + 1);
        const cursorDate = cursor || tomorrow.toISOString().split('T')[0];
        const distinctDatesResult = await db.query(`SELECT DISTINCT TO_CHAR(date, 'YYYY-MM-DD') as date
       FROM records
       WHERE user_id = $1 AND date < $2
       ORDER BY date DESC
       LIMIT $3`, [userId, cursorDate, limit + 1]);
        ...
        const recordsResult = await db.query(`SELECT
        id::text, type, category, sub_category as "subCategory", category_icon as "categoryIcon",
        amount, remark, TO_CHAR(date, 'YYYY-MM-DD HH24:MI:SS') as date, account, is_import as "isImport"
       FROM records
       WHERE user_id = $1 AND TO_CHAR(date, 'YYYY-MM-DD') = ANY($2)
       ORDER BY date DESC, created_at DESC`, [userId, targetDates]);
        ...
        const data = targetDates.map(date => ({
            date,
    ...
    // 获取报表统计数据
    async getReportData(userId, year, month) {
        const now = new Date();
       ...
       GROUP BY date
       ORDER BY date ASC`, [userId, startDate, endDate]);
        // 获取分类统计 - 支出
        const expenseCategoryResult = await db.query(`SELECT 
        category,
       ...
       ORDER BY amount DESC`, [userId, startDate, endDate]);
        // 获取分类统计 - 收入
        const incomeCategoryResult = await db.query(`SELECT 
        category,
        category_icon as "categoryIcon",
        COALESCE(SUM(amount), 0) as amount,
        COUNT(*) as count
       ...
       ORDER BY amount DESC`, [userId, startDate, endDate]);
        // 计算总支出和总收入
        const totalExpense = expenseCategoryResult.rows.reduce((sum, row) => sum + this.getFloat(row, 'amount'), 0);
        const totalIncome = incomeCategoryResult.rows.reduce((sum, row) => sum + this.getFloat(row, 'amount'), 0);
        // 构建分类统计数据（包含百分比）
        const expenseCategoryStats = expenseCategoryResult.rows.map(row => ({
        ...
        const incomeCategoryStats = incomeCategoryResult.rows.map(row => ({
            category: row.category,
            categoryIcon: row.categoryIcon,
            type: 'income',
            amount: this.getFloat(row, 'amount'),
    ...
    // 账单筛选查询
    async getBillsWithFilter(userId, params) {
        let query = `
      SELECT
        id::text, type, category, sub_category as "subCategory", category_icon as "categoryIcon",
        amount, remark, TO_CHAR(date, 'YYYY-MM-DD HH24:MI:SS') as date, account, is_import as "isImport"
    ...
    // 生成定时记账记录
    generateRecurringDates(frequency, startDate, endDate) {
        const dates = [];
    ...
    // 创建定时记账记录
    async createRecurringRecords(userId, data) {
        // 计算结束日期
        ...
        try {
            ...
            for (const date of dates) {
                try {
                    await client.query(`INSERT INTO records (type, category, sub_category, category_icon, amount, remark, date, account, user_id) 
    ...
    async createRecord(userId, data) {
        ...
        const result = await db.query(`INSERT INTO records (type, category, sub_category, category_icon, amount, remark, date, account, user_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id::text, type, category, sub_category as "subCategory", category_icon as "categoryIcon",
                 amount, remark, TO_CHAR(date, 'YYYY-MM-DD HH24:MI:SS') as date, account, is_import as "isImport"`, [data.type, data.category, data.subCategory || null, data.categoryIcon, data.amount, data.remark, formattedDate, data.account, userId]);
    ...
    async batchImportRecords(userId, records) {
        const client = await db.getClient();
        try {
            await client.query('BEGIN');
            let success = 0;
            ...
            for (let i = 0; i < records.length; i++) {
                ...
                try {
                    ...
                    await client.query(`INSERT INTO records (type, category, sub_category, category_icon, amount, remark, date, account, is_import, user_id)
    ...
    async deleteImportRecords(userId) {
        const result = await db.query(`DELETE FROM records WHERE user_id = $1 AND is_import = TRUE RETURNING id`, [userId]);
    ...
    async updateRecord(userId, id, data) {
        ...
        const result = await db.query(`UPDATE records
       ...
       RETURNING id::text, type, category, sub_category as "subCategory", category_icon as "categoryIcon",
                 amount, remark, TO_CHAR(date, 'YYYY-MM-DD HH24:MI:SS') as date, account, is_import as "isImport"`, [data.type, data.category, data.subCategory, data.categoryIcon, data.amount, data.remark, formattedDate, data.account, id, userId]);
    ...
    async deleteRecord(userId, id) {
        ...
        const result = await db.query(`DELETE FROM records WHERE id = $1 AND user_id = $2 RETURNING id`, [id, userId]);
    ...
    // 查找重复记录（预览）
    async findDuplicateRecords(userId) {
        // 查询所有记录，用于检测重复
        const result = await db.query(`SELECT
    ...
    // 删除重复记录
    async deduplicateRecords(userId) {
        ...
        try {
            ...
            for (const group of preview.duplicateGroups) {
                ...
                if (idsToDelete.length > 0) {
                    const result = await client.query(`DELETE FROM records 
             WHERE user_id = $1 AND id = ANY($2)`, [userId, idsToDelete]);
