...
export class TemplateService extends BaseService {
    ...
    async getAllTemplates(userId) {
        const result = await db.query(`SELECT id::text, name, type, category, sub_category, category_icon, amount, remark, account,
    ...
    async createTemplate(userId, data) {
        const result = await db.query(`INSERT INTO record_templates
    ...
    async updateTemplate(userId, id, data) {
        const result = await db.query(`UPDATE record_templates
    ...
    async deleteTemplate(userId, id) {
        const result = await db.query(`DELETE FROM record_templates WHERE id = $1 AND user_id = $2 RETURNING id`, [id, userId]);
        return result.rowCount ? result.rowCount > 0 : false;
    }
    // 使用模板创建记录
    async useTemplate(userId, templateId, data) {
        const templateResult = await db.query(`SELECT name, type, category, sub_category, category_icon, amount, remark, account
       FROM record_templates
       WHERE id = $1 AND user_id = $2`, [templateId, userId]);
