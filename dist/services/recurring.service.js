...
export class RecurringService extends BaseService {
    ...
    async getAllRecurring(userId) {
        const result = await db.query(`SELECT id::text, type, category, sub_category, category_icon, amount, remark,
    ...
    async createRecurring(userId, data) {
        ...
        const result = await db.query(`INSERT INTO recurring_records
    ...
    async updateRecurring(userId, id, data) {
        ...
        if (data.frequency || data.startDate) {
            const current = await db.query(`SELECT frequency, start_date FROM recurring_records WHERE id = $1 AND user_id = $2`, [id, userId]);
        ...
        const result = await db.query(`UPDATE recurring_records
    ...
    async deleteRecurring(userId, id) {
        const result = await db.query(`DELETE FROM recurring_records WHERE id = $1 AND user_id = $2 RETURNING id`, [id, userId]);
        return result.rowCount ? result.rowCount > 0 : false;
    }
    // 切换启用/禁用状态
    async toggleRecurring(userId, id) {
        const result = await db.query(`UPDATE recurring_records
