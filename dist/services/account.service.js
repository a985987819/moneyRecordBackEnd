...
export class AccountService extends BaseService {
    ...
    async getAllAccounts(userId) {
        const result = await db.query(`SELECT id::text, name, type, icon, balance, initial_balance, is_default, color,
    ...
    async updateAccount(userId, id, data) {
        ...
        try {
            ...
            // 如果设置为默认账户，先取消其他默认账户
            if (data.isDefault) {
                await client.query(`UPDATE accounts SET is_default = FALSE WHERE user_id = $1 AND id != $2`, [userId, id]);
            }
            const result = await client.query(`UPDATE accounts
    ...
    // 调整余额
    async adjustBalance(userId, id, data) {
        const client = await db.getClient();
        try {
            await client.query('BEGIN');
            const result = await client.query(`UPDATE accounts
         ...
         RETURNING id::text, name, type, icon, balance, initial_balance, is_default, color,
                   TO_CHAR(created_at, 'YYYY-MM-DD HH24:MI:SS') as created_at,
                   TO_CHAR(updated_at, 'YYYY-MM-DD HH24:MI:SS') as updated_at`, [data.newBalance, id, userId]);
            if (result.rows.length === 0) {
                await client.query('ROLLBACK');
                return null;
            }
            // 记录余额调整历史
            await client.query(`INSERT INTO account_adjustments (account_id, new_balance, remark, user_id)
         VALUES ($1, $2, $3, $4)`, [id, data.newBalance, data.remark || null, userId]);
            await client.query('COMMIT');
            return this.mapToResponse(result.rows[0]);
        }
        catch (error) {
    ...
    async getSummary(userId) {
       ...
       WHERE user_id = $1`, [userId]);
        // 按类型统计
        const byTypeResult = await db.query(`SELECT type, COALESCE(SUM(balance), 0) as total
       FROM accounts
       WHERE user_id = $1
