...
export class CategoryController {
    async getExpenseCategories(c) {
        ...
        try {
            ...
            logger.error(`获取支出分类失败`, error, { userId: user.userId });
            return c.json({ error: '获取分类失败' }, 500);
        }
    }
    async getIncomeCategories(c) {
        const user = c.get('user');
        try {
            ...
            logger.error(`获取收入分类失败`, error, { userId: user.userId });
            return c.json({ error: '获取分类失败' }, 500);
        }
    }
    async getAllCategories(c) {
        ...
        try {
            ...
            logger.error(`获取所有分类失败`, error, { userId: user.userId });
            return c.json({ error: '获取分类失败' }, 500);
        }
    }
    async createCategory(c) {
        ...
        try {
            ...
            logger.error(`创建分类失败`, error, { userId: user.userId });
    ...
    async updateCategory(c) {
        ...
        try {
            ...
            logger.error(`更新分类失败`, error, { userId: user.userId, categoryId: id });
    ...
    async deleteCategory(c) {
        ...
        try {
            ...
            logger.error(`删除分类失败`, error, { userId: user.userId, categoryId: id });
