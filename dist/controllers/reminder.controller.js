...
export class ReminderController {
    ...
    async getAllReminders(c) {
        ...
        try {
            ...
            logger.error(`获取提醒列表失败`, error, { userId: user.userId });
            return c.json({ error: '获取提醒列表失败' }, 500);
        }
    }
    // 创建提醒
    async createReminder(c) {
        const user = c.get('user');
        try {
            ...
            logger.error(`创建提醒失败`, error, { userId: user.userId });
            return c.json({ error: '创建提醒失败' }, 500);
        }
    }
    // 更新提醒
    async updateReminder(c) {
        const user = c.get('user');
        ...
        try {
            ...
            logger.error(`更新提醒失败`, error, { userId: user.userId, id });
            return c.json({ error: '更新提醒失败' }, 500);
        }
    }
    // 删除提醒
    async deleteReminder(c) {
        const user = c.get('user');
        const id = c.req.param('id');
        if (!id) {
            return c.json({ error: 'ID参数缺失' }, 400);
        ...
        try {
            ...
            logger.error(`删除提醒失败`, error, { userId: user.userId, id });
            return c.json({ error: '删除提醒失败' }, 500);
        }
    }
    // 切换启用/禁用状态
    async toggleReminder(c) {
        const user = c.get('user');
        const id = c.req.param('id');
        if (!id) {
            return c.json({ error: 'ID参数缺失' }, 400);
        ...
        try {
            ...
            logger.error(`切换提醒状态失败`, error, { userId: user.userId, id });
