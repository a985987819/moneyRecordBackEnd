...
export class BudgetController {
    ...
    async getCurrentBudget(c) {
        ...
        try {
            ...
            logger.error('获取当前月预算失败', error, { userId: user.userId });
            return c.json({ error: '获取预算失败' }, 500);
        }
    }
    // 获取指定月份预算
    async getBudgetByMonth(c) {
        const user = c.get('user');
        try {
            ...
            logger.error('获取指定月份预算失败', error, { userId: user.userId });
            return c.json({ error: '获取预算失败' }, 500);
        }
    }
    // 设置预算
    async setBudget(c) {
        ...
        try {
            ...
            logger.error('设置预算失败', error, { userId: user.userId });
            return c.json({ error: '设置预算失败' }, 500);
        }
    }
    // 删除预算
    async deleteBudget(c) {
        const user = c.get('user');
        try {
            const query = c.req.query();
            ...
            logger.error('删除预算失败', error, { userId: user.userId });
            return c.json({ error: '删除预算失败' }, 500);
        }
    }
    // 获取预算统计
    async getBudgetStats(c) {
        const user = c.get('user');
        try {
            ...
            logger.error('获取预算统计失败', error, { userId: user.userId });
            return c.json({ error: '获取统计失败' }, 500);
        }
    }
    // 获取最近几个月预算
    async getRecentBudgets(c) {
        const user = c.get('user');
        try {
            const query = c.req.query();
            ...
            logger.error('获取最近几个月预算失败', error, { userId: user.userId });
