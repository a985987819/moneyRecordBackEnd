...
export class SyncController {
    ...
    async uploadData(c) {
        ...
        try {
            ...
            logger.error(`数据上传失败`, error, { userId: user.userId });
            return c.json({ error: '数据上传失败' }, 500);
        }
    }
    // 下载数据
    async downloadData(c) {
        const user = c.get('user');
        const version = c.req.query('version');
        const versionNum = version ? safeParseInt(version, undefined) : undefined;
        ...
        try {
            ...
            logger.error(`数据下载失败`, error, { userId: user.userId });
    ...
    async getVersions(c) {
        ...
        try {
            ...
            logger.error(`获取版本列表失败`, error, { userId: user.userId });
    ...
    async restoreVersion(c) {
        ...
        try {
            ...
            logger.error(`版本恢复失败`, error, { userId: user.userId, versionId });
