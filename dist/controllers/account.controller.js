...
export class AccountController {
    ...
    async getAllAccounts(c) {
        ...
        try {
            ...
            logger.error(`获取账户列表失败`, error, { userId: user.userId });
    ...
    async createAccount(c) {
        ...
        try {
            ...
            logger.error(`创建账户失败`, error, { userId: user.userId });
    ...
    async updateAccount(c) {
        ...
        try {
            ...
            logger.error(`更新账户失败`, error, { userId: user.userId, id });
    ...
    async deleteAccount(c) {
        ...
        try {
            ...
            logger.error(`删除账户失败`, error, { userId: user.userId, id });
            return c.json({ error: '删除账户失败' }, 500);
        }
    }
    // 调整余额
    async adjustBalance(c) {
        const user = c.get('user');
        const id = c.req.param('id');
        if (!id) {
            return c.json({ error: 'ID参数缺失' }, 400);
        ...
        try {
            ...
            logger.error(`调整余额失败`, error, { userId: user.userId, id });
    ...
    async getSummary(c) {
        ...
        try {
            ...
            logger.error(`获取账户统计失败`, error, { userId: user.userId });
