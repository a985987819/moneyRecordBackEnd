...
export class RecurringController {
    ...
    async getAllRecurring(c) {
        ...
        try {
            ...
            logger.error(`获取周期记账列表失败`, error, { userId: user.userId });
            return c.json({ error: '获取周期记账列表失败' }, 500);
        }
    }
    // 创建周期记账
    async createRecurring(c) {
        const user = c.get('user');
        try {
            ...
            logger.error(`创建周期记账失败`, error, { userId: user.userId });
            return c.json({ error: '创建周期记账失败' }, 500);
        }
    }
    // 更新周期记账
    async updateRecurring(c) {
        const user = c.get('user');
        ...
        try {
            ...
            logger.error(`更新周期记账失败`, error, { userId: user.userId, id });
            return c.json({ error: '更新周期记账失败' }, 500);
        }
    }
    // 删除周期记账
    async deleteRecurring(c) {
        const user = c.get('user');
        const id = c.req.param('id');
        if (!id) {
            return c.json({ error: 'ID参数缺失' }, 400);
        ...
        try {
            ...
            logger.error(`删除周期记账失败`, error, { userId: user.userId, id });
            return c.json({ error: '删除周期记账失败' }, 500);
        }
    }
    // 切换启用/禁用状态
    async toggleRecurring(c) {
        const user = c.get('user');
        const id = c.req.param('id');
        if (!id) {
            return c.json({ error: 'ID参数缺失' }, 400);
        ...
        try {
            ...
            logger.error(`切换周期记账状态失败`, error, { userId: user.userId, id });
    ...
    async getSummary(c) {
        ...
        try {
            ...
            logger.error(`获取周期记账统计失败`, error, { userId: user.userId });
